...
try:
    ...
    tesseract_path = "/usr/bin/tesseract"  # Default path for Tesseract on Linux
    if not os.path.exists(tesseract_path):
        raise FileNotFoundError("Tesseract OCR is not installed.")
    pytesseract.pytesseract.tesseract_cmd = tesseract_path
except (ImportError, FileNotFoundError) as e:
    st.error("Tesseract OCR is missing. Please install it using: `sudo apt-get install tesseract-ocr`.")
    st.stop()
